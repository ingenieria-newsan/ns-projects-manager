...
module.exports = () =>
{
    ...
    router.post( '/editar/:id', async (req,res) => 
    {
        ...
        // Etiqueta
        if ( data.productRatingLabel ) { data.productRatingLabel = { required:true }; } else { data.productRatingLabel = { required:false }; }
        
        // Caja individual
        if ( data.productGiftBox ) { data.productGiftBox = { required:true }; } else { data.productGiftBox = { required:false }; }

        // Manual de usuario
        if ( data.productUserManual ) { data.productUserManual = { required:true }; } else { data.productUserManual = { required:false }; }

        // Caja maaster
        if ( data.productMasterBox ) { data.productMasterBox = { required:true }; } else { data.productMasterBox = { required:false }; }

        // Desarrollo de software
        if ( data.productSoftware ) { data.productSoftware = { required:true }; } else { data.productSoftware = { required:false }; }

        // BOM
        if ( data.productBom ) { data.productBom = { required:true }; } else { data.productBom = { required:false }; }
        if ( data.productEnacom ) { data.productEnacom = { required:true }; } else { data.productEnacom = { required:false }; }
        if ( data.productEficiency ) { data.productEficiency = { required:true }; } else { data.productEficiency = { required:false }; }
        if ( data.projectDeleted ) { data.projectDeleted = true; } else { data.projectDeleted = false; }
